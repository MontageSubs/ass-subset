...
function parseASSText(text, id, forceHasBOM) {
  ...
  for (let li = 0; li < totalLines; li++) {
    ...
    if (section === '[script info]') {
      ...
      const randM = t.match(/^;\s*Font Subset:\s*([A-Z0-9]{7,8})\s*-\s*(.+)$/);
  ...
  const isRandFontName = (n) => /^[A-Z0-9]{7,8}$/.test(n);
  const fontInternalMapRe = /FontSubsetMap:\s*\{original:\s*(.+?),\s*subset:\s*([A-Z0-9]{7,8}),\s*ass-subset:\s*([\d.]+)\}/;
...
async function doConvert(data, id) {
  ...
  if (parsed.randMapMissing && parsed.embeddedFonts) {
    const isRandFontNameLocal = (n) => /^[A-Z0-9]{7,8}$/.test(n);
