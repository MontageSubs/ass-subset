...
let APP_VERSION = '2.7';
const TOOL_DESIGNER = 'ASS Subsetter (MontageSubs)';
const TOOL_DESIGNER_MARKER_RE = /^(?:ASS Subsetter \(MontageSubs\)|MontageSubs \(ASS Subsetter\))$/i;
const TOOL_COPYRIGHT_MARKER_RE = /^(?:Subsetted via|Generated by)\s+ASS Subsetter\b/i;
const TOOL_MANUFACTURER_MARKER_RE = /^MontageSubs$/i;
function stripToolSegments(str, markerRe) {
  if (!str) return '';
  return str.split(/;\s*/).map(s => s.trim()).filter(s => s && !markerRe.test(s)).join('; ');
}
const EM = 1024;
const TARGET = 820;
...
function parseASSText(text, id, forceHasBOM) {
  ...
  for (let li = 0; li < totalLines; li++) {
    ...
    if (section === '[assfonts rename info]') {
      const renameM = t.match(/^(.+?)\s*-{2,}\s*([A-Za-z0-9]{8})\s*$/);
      if (renameM) { randFontMap[renameM[2].trim()] = renameM[1].trim(); }
    }
  ...
  const applyRandResolution = (map) => {
    for (const [rk, ov] of Object.entries(map)) {
      ...
      for (const ekKey of matchingKeys) {
        const suffixMatch = ekKey.match(/(_B0|_I0|_BI0|_0)\.ttf$/i);
        const newKey = suffixMatch ? ov + suffixMatch[1] + '.ttf' : ov + ekKey.slice(rk.length);
  ...
  const potentialRandEmbedded = Object.keys(embeddedFonts).some(embName => {
    ...
    return !isAnyDrawFont(baseEmbName) && !!randCodeOf(baseEmbName);
  });
  ...
  if (Object.keys(randFontMap).length > 0 || potentialRandEmbedded) {
    ...
    for (const [embName, embLines] of Object.entries(embeddedFonts)) {
      ...
      if (!randCodeOf(baseEmbName) && !randFontMap[baseEmbName]) continue;
      try {
    ...
    for (const [embName] of Object.entries(embeddedFonts)) {
      const baseEmbName = embName.replace(/(_B0|_I0|_BI0|_0)\.ttf$/i, '');
      if (isAnyDrawFont(baseEmbName)) continue;
      const randCode = randCodeOf(baseEmbName);
      if (randCode && !resolvedRandKeys.has(randCode.toLowerCase())) {
        randMapMissing = true;
      }
    }
...
function buildDrawingFont(uniqueDrawingsArray, existingFontBuffer, referencedCharsArray, id, familyName, onProgress, fileName) {
  ...
  font.names = {
    copyright: { en: `MontageSubs; Generated by ASS Subsetter ${APP_VERSION} (${PROJECT_URL}) on ${drawDateStr}` },
    ...
    designer: { en: TOOL_DESIGNER },
    description: { en: `ASS Subsetter ${APP_VERSION} (${PROJECT_URL}) - Drawing Command Font generated for ${fileName || 'unknown subtitle'}` },
    manufacturerURL: { en: PROJECT_URL },
    designerURL: { en: PROJECT_URL },
    license: { en: `MIT; ${TOOL_DESIGNER}` },
...
function buildResidualDrawFont(existingFontBuffer, retainChars, familyName) {
  ...
  font.names = {
    copyright: { en: `MontageSubs; Generated by ASS Subsetter ${APP_VERSION} (${PROJECT_URL}) on ${buildSubsetDateString()}` },
    ...
    manufacturer: { en: 'MontageSubs' }, designer: { en: TOOL_DESIGNER },
    description: { en: `ASS Subsetter ${APP_VERSION} (${PROJECT_URL}) - Drawing Command Font (partial residual)` },
    manufacturerURL: { en: PROJECT_URL }, designerURL: { en: PROJECT_URL },
    license: { en: `MIT; ${TOOL_DESIGNER}` }, licenseURL: { en: PROJECT_URL },
  };
  const rawTtf = repairFontBuffer(new Uint8Array(font.toArrayBuffer()));
...
function modifyNameTable(buffer, newNames) {
  ...
  const resolveValue = (entries, platformID, encodingID) => {
    let str = entries['en'] || Object.values(entries).find(v => typeof v === 'string') || '';
    ...
    if (/[^\x00-\xFF]/.test(str)) {
      str = Object.values(entries).find(v => typeof v === 'string' && /^[\x00-\xFF]*$/.test(v)) || '';
      if (!str) return null;
    }
    const bytes = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (newNames._familyValueMap && FAMILY_NAME_IDS.has(nameID)) {
      ...
      if (mapped !== undefined) {
        matched = true;
        bytes = resolveValue({ en: mapped }, platformID, encodingID);
      }
    }
    if (!matched) for (const [field, entries] of Object.entries(newNames)) {
...
async function subsetFont(fontBuffer, charArray, fontName, isTTC, targetWeight, ttcIndex, id, wantAscii, wantFullFont, aliasNames) {
  ...
  const baseCopyrightStr = stripToolSegments(getOrigNameStr(origCopyright), TOOL_COPYRIGHT_MARKER_RE);
  const copyrightStr = `${baseCopyrightStr || 'MontageSubs'}; Generated by ASS Subsetter ${APP_VERSION} (${PROJECT_URL}) on ${dateStr}`;
  ...
  const baseDesignerStr = stripToolSegments(getOrigNameStr(origDesigner), TOOL_DESIGNER_MARKER_RE);
  const designerStr = baseDesignerStr ? `${baseDesignerStr}; ${TOOL_DESIGNER}` : TOOL_DESIGNER;
  const designerEntry = {};
  ...
  const baseRealManufacturerStr = stripToolSegments(getOrigNameStr(origRealManufacturer), TOOL_MANUFACTURER_MARKER_RE);
  ...
  const urlEntry = {};
  for (const lk of langKeysForFamily) {
    descriptionEntry[lk] = `ASS Subsetter ${APP_VERSION} (${PROJECT_URL})`;
...
function rewriteASS(rawContent, opts, id) {
  ...
  for (let i = 0; i < totalBlocks; i++) {
    ...
    if (header === 'fonts' || header === 'assfonts rename info') continue;
...
async function doConvert(data, id) {
  ...
  if (parsed.randMapMissing && parsed.embeddedFonts) {
    for (const embName of Object.keys(parsed.embeddedFonts)) {
      ...
      if (isAnyDrawFont(baseEmbName)) continue;
      const randCode = randCodeOf(baseEmbName);
      if (randCode && !parsed.randFontMap?.[randCode]) {
        unresolvableRandBases.add(randCode.toLowerCase());
      }
    }
  }
  if (options.wantFont) {
  ...
  const freshFamilyNameSet = new Set(embeddedFonts.flatMap(ef =>
    [ef.name.replace(/(_B|_I|_BI)$/, ''), ...(ef.aliasNames || [])]
  ).map(n => n.toLowerCase()));
  ...
  if (parsed.embeddedFonts) {
    for (const [name, lines] of Object.entries(parsed.embeddedFonts)) {
      ...
      const identityName = randCodeOf(baseName) || baseName;
      const baseNameLower = identityName.toLowerCase();
      ...
      if (unresolvableRandBases.has(baseNameLower)) {
        ...
        strippedNames.add(identityName);
        continue;
      }
      try {
        const buf = assUUDecode(lines);
        let isSuperseded = false;
        if (freshFamilyNameSet.size > 0) {
          try {
            const meta = extractMetaFromTables(buf.buffer, locateTableInBuffer(buf.buffer, 0));
            isSuperseded = [...meta.allNames].some(n => freshFamilyNameSet.has(n.toLowerCase()));
          } catch (_) { }
        }
        if (!isSuperseded) {
          finalEmbeddedFonts.push({ name: identityName, ttf: buf, weightSlot: weightSlotFromName });
    ...
    for (const ef of finalEmbeddedFonts) {
      ...
      if (isFresh) {
        ...
        if (entry) {
          ...
          const descEntry = { en: `${mapLines}; ASS Subsetter ${APP_VERSION} (${PROJECT_URL})` };
  ...
  if (options.wantStrip && unresolvableRandBases.size > 0 && parsed.embeddedFonts) {
    for (const [name, lines] of Object.entries(parsed.embeddedFonts)) {
      ...
      const baseName = slotMatch ? slotMatch[1] : name.replace(/_\d+\.ttf$/i, '');
      const identityName = randCodeOf(baseName) || baseName;
      if (unresolvableRandBases.has(identityName.toLowerCase())) {
