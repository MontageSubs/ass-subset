...
**ASS 子集化工具** 是由 MontageSubs 开发的开源浏览器工具，用于在 ASS/SSA 字幕文件中管理字体嵌入与绘图指令。ASS/SSA 格式允许使用自定义字体来增强视觉效果，但跨平台安装字体不便。本工具在浏览器中直接实现字体嵌入，无需额外软件，跨平台可用；同时支持**字体子集化**，仅保留字幕实际使用的字符以进一步减小文件体积，并可将重复的绘图指令转换为字体以最大化压缩效果。
