...
**ASS Subsetter** 是由 MontageSubs 开发的开源浏览器工具，用于在 ASS/SSA 字幕文件中管理字体嵌入与绘图指令。ASS/SSA 格式允许使用自定义字体来增强视觉效果，但跨平台安装字体不便。本工具在浏览器中直接实现字体嵌入，无需额外软件，跨平台可用；同时支持**字体子集化**，仅保留字幕实际使用的字符以进一步减小文件体积，并可将重复的绘图指令转换为字体以最大化压缩效果。

## 核心特色

- **浏览器中直接处理** — 所有文件在本地处理，支持离线使用，保护隐私，无需安装额外软件。  
- **预览与下载** — 处理后可即时预览并对比优化前后，满意后下载优化后的字幕与内嵌字体用于本地播放。  
- **符合无障碍标准** — 完整支持键盘导航与屏幕阅读器，满足 WCAG 及相关国际无障碍设计要求。  
- **精心设计的界面** — 现代化前端与直观交互，兼具美观与易用性。
