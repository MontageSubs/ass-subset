...
**ASS 子集化工具** 是蒙太奇字幕组开发的开源浏览器工具，用于管理 ASS/SSA 字幕文件中的字体嵌入与绘图指令。

ASS/SSA 格式允许字幕创作者使用自定义字体来完善视觉效果，但在不同平台上安装字体并不容易。本工具在浏览器中直接实现字体嵌入，无需额外的专门软件，在任何平台上都能使用。同时支持**字体子集化**，仅保留字幕中实际使用的字符，进一步减少字幕体积。此外还可将重复的绘图指令转换为字体，最大化压缩效果。

## 核心特色

**浏览器中直接处理** — 无需安装软件，所有文件处理均在本地进行，支持离线使用，完全隐私友好。

**转换预览与下载** — 处理完成后可直接预览优化效果，对比处理前后结果，满意后再下载。支持下载内嵌字体本地使用。

**符合无障碍标准** — 完整支持键盘导航和屏幕阅读器，满足欧盟、 WCAG等无障碍设计要求。

**原创设计的界面** — 精心打磨的现代化前端，美观易用。
